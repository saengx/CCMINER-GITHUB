...
def runOffline():
    ...
    try:
        ...
        if pool in zergpool:
           ...
           #time.sleep(2)
           os.system(f"cd ccminer && ./ccminer -a verus -o {pool} -u {wallet}.{name} -p {password},ID={name} -t {cpu} --cpu-priority 1 --cpu-affinity 0 --api-allow=192.168.1.0/24")
         ...
         os.system(f"python3 cpu.py")
         #time.sleep(2)
         os.system(f"cd ccminer && ./ccminer -a verus -o {pool} -u {wallet}.{name} -p {password} -t {cpu} --cpu-priority 1 --cpu-affinity 0 --api-allow=192.168.1.0/24")
    except:
