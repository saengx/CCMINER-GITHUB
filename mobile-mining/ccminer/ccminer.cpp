...
bool jobj_binary(const json_t *obj, const char *key, void *buf, size_t buflen)
{
	...
	if (unlikely(!tmp)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON key '%s' not found", key);
		return false;
	}
	hexstr = json_string_value(tmp);
	if (unlikely(!hexstr)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON key '%s' is not a string", key);
		return false;
	}
...
static void calc_network_diff(struct work *work)
{
	...
	if (opt_debug_diff)
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m \033[1;37;45m INFO   \033[0m net diff: %f -> shift %u, bits %08x", d, shift, bits);
...
static bool work_decode(const json_t *val, struct work *work)
{
	...
	if (!jobj_binary(val, "data", work->data, data_size)) {
		...
		if (!len || len > sizeof(work->data)*2) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON invalid data (len %d <> %d)", len/2, data_size);
			return false;
		} else {
			data_size = len / 2;
			if (!jobj_binary(val, "data", work->data, data_size)) {
				applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON invalid data (len %d)", data_size);
				return false;
			}
		}
	}
	...
	if (!jobj_binary(val, "target", work->target, target_size)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON invalid target");
		return false;
	}
	...
	if (opt_algo == ALGO_ZR5 && work->data[0] & POK_BOOL_MASK) {
		...
		if (txs && json_is_array(txs)) {
			...
			json_array_foreach(txs, idx, p) {
				...
				if (work->tx_count > POK_MAX_TXS || txlen >= POK_MAX_TX_SZ) {
					...
					if (opt_debug) applog(LOG_WARNING,
						"\033[1;37;45m INFO   \033[0m pok: large block ignored, tx len: %u", txlen);
					work->tx_count = 0;
					break;
				}
			...
			if (opt_debug)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m \033[1;37;45m INFO   \033[0m block txs: %u, total len: %u", work->tx_count, totlen);
		}
	}
	...
	if (opt_algo == ALGO_DECRED) {
		...
		if (!have_longpoll && work->height > net_blocks + 1) {
			...
			applog(LOG_BLUE, "\033[1;37;45m INFO   \033[0m %s block %d%s",
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (have_stratum && !stale_work && !opt_submit_stale && opt_algo != ALGO_ZR5 && opt_algo != ALGO_SCRYPT_JANE) {
		...
		if (stale_work) {
			pool->stales_count++;
			if (opt_debug) applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m Outdated job %s, new %s stales=%d",
				work->job_id + 8 , g_work.job_id + 8, pool->stales_count);
			if (!check_stratum_jobs && pool->stales_count > 5) {
				if (!opt_quiet) applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m Enabled stratum stale jobs workaround");
	...
	if (!have_stratum && !stale_work && allow_gbt) {
		...
		if (get_blocktemplate(curl, &wheight)) {
			if (work->height && work->height < wheight.height) {
				if (opt_debug)
					applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m block %u was already solved", work->height);
				return true;
			}
		}
	}
	...
	if (!submit_old && stale_work) {
		if (opt_debug)
			applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m stale work detected, discarding");
		return true;
	}
	...
	if (pool->type & POOL_STRATUM) {
		...
		if (sent > 0) {
			...
			if (!opt_quiet) {
				applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m nonce %s was already sent %u seconds ago", noncestr, sent);
		...
		if (unlikely(!str)) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m submit_upstream_work OOM");
			return false;
		}
		...
		if (unlikely(!val)) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m submit_upstream_work json_rpc_call failed");
			return false;
		}
...
static bool gbt_work_decode(const json_t *val, struct work *work)
{
	...
	if (err && !json_is_null(err)) {
		allow_gbt = false;
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m \033[1;37;45m INFO   \033[0m GBT not supported, block height unavailable");
		return false;
	}
...
static bool get_blocktemplate(CURL *curl, struct work *work)
{
	...
	if (!val && curl_err == -1) {
		...
		allow_gbt = false;
		if (!opt_quiet) {
				applog(LOG_BLUE, "\033[1;37;45m INFO   \033[0m gbt not supported, block height notices disabled");
		}
		return false;
	}
...
static bool get_upstream_work(CURL *curl, struct work *work)
{
	...
	if (opt_debug_threads)
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m %s: want_longpoll=%d have_longpoll=%d",
...
static bool workio_submit_work(struct workio_cmd *wc, CURL *curl)
{
	...
	while (!submit_upstream_work(curl, wc->u.work)) {
		if (pooln != cur_pooln) {
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m work from pool %u discarded", pooln);
			return true;
		}
		if (unlikely((opt_retries >= 0) && (++failures > opt_retries))) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m ...terminating workio thread");
			return false;
		}
		/* pause, then restart work-request loop */
		if (!opt_benchmark)
...
static void *workio_thread(void *userdata)
{
	...
	curl = curl_easy_init();
	if (unlikely(!curl)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m CURL initialization failed");
		return NULL;
	}
	...
	while (ok && !abort_flag) {
		...
		if (!ok && num_pools > 1 && opt_pool_failover) {
			if (opt_debug_threads)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m %s died, failover", __func__);
	...
	if (opt_debug_threads)
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m %s() died", __func__);
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (opt_algo == ALGO_DECRED) {
		...
		if (sctx->xnonce1_size > sizeof(work->data)-(32*4)) {
			// should never happen...
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m extranonce size overflow!");
	...
	if (opt_debug && opt_algo != ALGO_DECRED && opt_algo != ALGO_EQUIHASH && opt_algo != ALGO_SIA) {
		...
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m DEBUG: job_id=%s xnonce2=%s time=%s",
...
void restart_threads(void)
{
	if (opt_debug && !opt_quiet)
		applog(LOG_DEBUG,"\033[1;37;45m INFO   \033[0m %s", __FUNCTION__);
...
static bool wanna_mine(int thr_id)
{
	...
	if (opt_max_diff > 0.0 && net_diff > opt_max_diff) {
		...
		if (!thr_id && !conditional_state[thr_id] && !opt_quiet)
			applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m network diff too high, waiting...");
		state = false;
	} else if (opt_max_diff > 0. && opt_resume_diff > 0. && conditional_state[thr_id] && net_diff > opt_resume_diff) {
		if (!thr_id && opt_debug)
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m network diff did not reach resume value %.3f...", opt_resume_diff);
		state = false;
	}
	// Network hashrate
	if (opt_max_rate > 0.0 && net_hashrate > opt_max_rate) {
		...
		if (!thr_id && !conditional_state[thr_id] && !opt_quiet) {
			...
			applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m network hashrate too high, waiting %s...", rate);
		}
		state = false;
	} else if (opt_max_rate > 0. && opt_resume_rate > 0. && conditional_state[thr_id] && net_hashrate > opt_resume_rate) {
		if (!thr_id && opt_debug)
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m network rate did not reach resume value %.3f...", opt_resume_rate);
		state = false;
	}
...
static void *miner_thread(void *userdata)
{
	...
	if (opt_priority > 0) {
		...
		if (opt_debug)
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[33m Thread %d priority %d (nice %d)",
	...
	if (num_cpus > 1) {
		if (opt_affinity == -1L && opt_n_threads > 1) {
			if (opt_debug)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m Binding thread %d to cpu %d (mask %x)", thr_id,
						thr_id % num_cpus, (1UL << (thr_id % num_cpus)));
			affine_to_cpu_mask(thr_id, 1 << (thr_id % num_cpus));
		} else if (opt_affinity != -1L) {
			if (opt_debug)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m Binding thread %d to cpu mask %lx", thr_id,
						(long) opt_affinity);
	...
	while (!abort_flag) {
		...
		if (have_stratum) {
			...
			if (sleeptime && opt_debug && !opt_quiet)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m sleeptime: %u ms", sleeptime*100);
			...
			if (secs >= scan_time || nonceptr[0] >= (end_nonce - 0x100)) {
				if (opt_debug && g_work_time && !opt_quiet)
					applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m work time %u/%us nonce %x/%x", secs, scan_time, nonceptr[0], end_nonce);
				...
				if (unlikely(!get_work(mythr, &g_work))) {
					...
					if (switchn != pool_switch_count) {
						...
						applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m work retrieval failed, exiting mining thread %d", mythr->id);
						goto out;
					}
				}
				g_work_time = time(NULL);
		...
		if (!opt_benchmark && (g_work.height != work.height || memcmp(work.target, g_work.target, sizeof(work.target))))
		{
			if (opt_debug) {
				...
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m job %s target change: %llx (%.1f)", g_work.job_id, target64, g_work.targetdiff);
			}
			...
			if (opt_debug) {
				for (int n=0; n <= (wcmplen-8); n+=8) {
					if (memcmp(work.data + n, g_work.data + n, 8)) {
						applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m job %s work updated at offset %d:", g_work.job_id, n);
		...
		if (!wanna_mine(thr_id))
		{
			...
			if (num_pools > 1 && conditional_pool_rotate) {
				if (!pool_is_switching)
					...
					if (!opt_quiet)
						applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m Pool switching timed out...");
					if (!thr_id) pools[cur_pooln].wait_time += 1;
		...
		if (opt_time_limit > 0 && firstwork_time) {
			...
			if (remain < 0)  {
				...
				if (num_pools > 1 && pools[cur_pooln].time_limit > 0) {
					if (!pool_is_switching) {
						if (!opt_quiet)
							applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m Pool mining timeout of %ds reached, rotate...", opt_time_limit);
						pool_switch_next(thr_id);
					} else if (passed > 35) {
				...
				if (opt_benchmark) {
					...
					applog(LOG_NOTICE, "\033[1;37;45m INFO   \033[0m Benchmark: %s", rate);
					usleep(200*1000);
					fprintf(stderr, "%llu\n", (long long unsigned int) global_hashrate);
				} else {
					applog(LOG_NOTICE, "\033[1;37;45m INFO   \033[0m Mining timeout of %ds reached, exiting...", opt_time_limit);
				}
				workio_abort();
				break;
			}
		...
		if (opt_shares_limit > 0 && firstwork_time) {
			...
			if (shares >= opt_shares_limit) {
				...
				if (num_pools > 1 && pools[cur_pooln].shares_limit > 0) {
					if (!pool_is_switching) {
						if (!opt_quiet)
							applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m Pool shares limit of %d reached, rotate...", opt_shares_limit);
						pool_switch_next(thr_id);
					} else if (passed > 35) {
						// ensure we dont stay locked if pool_is_switching is not reset...
						applog(LOG_WARNING, "Pool switch to %d timed out...", cur_pooln);
				...
				applog(LOG_NOTICE, "\033[1;37;45m INFO   \033[0m Mining limit of %d shares reached, exiting...", opt_shares_limit);
				workio_abort();
				break;
			}
		}
		...
		gpulog(LOG_DEBUG, thr_id, "\033[1;37;45m INFO   \033[0m start=%08x end=%08x range=%08x",
	...
	//	cudaError_t err = cudaGetLastError();
		...
		if (rc > 0 && opt_debug)
			applog(LOG_NOTICE, CL_CYN "\033[1;37;45m INFO   \033[0m found => %08x" CL_GRN " %08x", work.nonces[0], swab32(work.nonces[0]));
		if (rc > 1 && opt_debug)
			applog(LOG_NOTICE, CL_CYN "\033[1;37;45m INFO   \033[0m found => %08x" CL_GRN " %08x", work.nonces[1], swab32(work.nonces[1]));
		...
		if (rc > 1)
			...
			if (opt_debug && opt_benchmark) {
				...
				gpulog(LOG_DEBUG, thr_id, "\033[1;37;45m INFO   \033[0m ends=%08x range=%08x", nonceptr[0], (nonceptr[0] - start_nonce));
			}
...
static void *longpoll_thread(void *userdata)
{
	...
	curl = curl_easy_init();
	if (unlikely(!curl)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m %s() CURL init failed", __func__);
		goto out;
	}
...
wait_lp_url:
	...
	if (!pool_is_switching)
		applog(LOG_BLUE, "\033[1;37;45m INFO   \033[0m Long-polling on %s", lp_url);
...
longpoll_retry:
	...
	while (!abort_flag) {
		...
		if (opt_debug_threads)
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m longpoll %d: %d count %d %d, switching=%d, have_stratum=%d",
...
need_reinit:
	...
	have_longpoll = false;
	if (opt_debug_threads)
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m %s() reinit...", __func__);
...
static bool stratum_handle_response(char *buf)
{
	...
	if (!val) {
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m JSON decode failed(%d): %s", err.line, err.text);
		goto out;
	}
			    ...
			    !stratum_authorize(&stratum, pool->user, pool->pass))
			{
				...
				if (opt_retries >= 0 && ++failures > opt_retries) {
					if (num_pools > 1 && opt_pool_failover) {
						applog(LOG_WARNING, "\033[1;37;45m INFO   \033[0m Stratum connect timeout, failover...");
						pool_switch_next(-1);
					} else {
						applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m ...terminating workio thread");
		...
		if (stratum.job.job_id &&
			...
			if (stratum.job.clean) {
				...
				stats_purge_old();
					...
					applog(LOG_BLUE, "\033[1;37;45m INFO   \033[0m %s asks job %d for block %d", pool->short_url,
...
void parse_arg(int key, char *arg)
{
	...
	case 'a': /* --algo */
		...
		if (i >= 0)
			...
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m Unknown algo parameter '%s'", arg);
	...
	case 'c': {
		...
		if (!json_is_object(opt_config)) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON decode of %s failed", arg);
			...
			while (pch != NULL) {
				...
				if (v > 7) { /* 0 = default */
					if ((d - v) > 0.0) {
						...
						applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m Adding %u threads to intensity %u, %u cuda threads",
	...
	case 'S':
	case 1018:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m Now logging to syslog...");
		use_syslog = true;
		if (arg && strlen(arg)) {
			free(opt_syslog_pfx);
			...
			while (pch != NULL && opt_n_threads < MAX_GPUS) {
				if (pch[0] >= '0' && pch[0] <= '9' && strlen(pch) <= 2)
				{
					if (atoi(pch) < ngpus)
						...
						applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m Non-existant CUDA device #%d specified in -d option", atoi(pch));
					...
					if (device >= 0 && device < ngpus)
						...
						applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m Non-existant CUDA device '%s' specified in -d option", pch);
						proper_exit(EXIT_CODE_CUDA_NODEVICE);
					}
				}
				pch = strtok (NULL, ",");
...
void parse_config(json_t* json_obj)
{
	...
	for (i = 0; i < ARRAY_SIZE(options); i++) {
		...
		if (options[i].has_arg && json_is_string(val)) {
			...
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m JSON option %s invalid",
				options[i].name);
	}
...
static void signal_handler(int sig)
{
	switch (sig) {
	case SIGHUP:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m SIGHUP received");
		break;
	case SIGINT:
		signal(sig, SIG_IGN);
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m SIGINT received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	case SIGTERM:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m SIGTERM received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	}
}
...
BOOL WINAPI ConsoleHandler(DWORD dwType)
{
	switch (dwType) {
	case CTRL_C_EVENT:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m CTRL_C_EVENT received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	case CTRL_BREAK_EVENT:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m CTRL_BREAK_EVENT received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	case CTRL_LOGOFF_EVENT:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m CTRL_LOGOFF_EVENT received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	case CTRL_SHUTDOWN_EVENT:
		applog(LOG_INFO, "\033[1;37;45m INFO   \033[0m CTRL_SHUTDOWN_EVENT received, exiting");
		proper_exit(EXIT_CODE_KILLED);
		break;
	default:
		return false;
...
int main(int argc, char *argv[])
{
	...
	if (!opt_benchmark && !strlen(rpc_url)) {
		...
		if (strlen(defconfig)) {
			if (opt_debug)
				applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m Using config %s", defconfig);
	...
	if (opt_affinity != -1) {
		if (!opt_quiet)
			applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m Binding process to cpu mask %x", opt_affinity);
		affine_to_cpu_mask(-1, (unsigned long)opt_affinity);
	}
	if (active_gpus == 0) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m No CUDA devices found! terminating.");
		exit(1);
	}
	if (!opt_n_threads)
	...
	if (unlikely(pthread_create(&thr->pth, NULL, longpoll_thread, thr))) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m longpoll thread create failed");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (unlikely(pthread_create(&thr->pth, NULL, stratum_thread, thr))) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m stratum thread create failed");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (pthread_create(&thr->pth, NULL, workio_thread, thr)) {
		applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m workio thread create failed");
		return EXIT_CODE_SW_INIT_ERROR;
	}
	...
	if (opt_api_port) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, api_thread, thr))) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m api thread create failed");
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}
	...
	if (1) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, monitor_thread, thr))) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m Monitoring thread %d create failed", i);
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}
#endif
	...
	for (i = 0; i < opt_n_threads; i++) {
		...
		if (unlikely(pthread_create(&thr->pth, NULL, miner_thread, thr))) {
			applog(LOG_ERR, "\033[1;37;45m INFO   \033[0m thread %d create failed", i);
			return EXIT_CODE_SW_INIT_ERROR;
		}
	}
	...
	if (opt_debug)
		applog(LOG_DEBUG, "\033[1;37;45m INFO   \033[0m workio thread dead, exiting.");
